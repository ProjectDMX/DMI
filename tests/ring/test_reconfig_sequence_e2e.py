"""Reconfigure-sequence alignment guardrail.
...
This guards the toggle *reconfigure* path and is the safety net required before
any delta / lazy apply_toggle optimization: a wrong reconfigure (stale node,
wrong graph, bad diff seed) would desync and fail here.

Run:  CUDA_VISIBLE_DEVICES=<free gpu> python tests/ring/test_reconfig_sequence_e2e.py
