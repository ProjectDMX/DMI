...
= 4 cells.  Eager cells should trivially have zero graph breaks (no
...
CUDA-graph caches don't leak between cells.
...
pytestmark = [pytest.mark.gpu, pytest.mark.hf]
...
# ---------------------------------------------------------------------------
# The subprocess runner prints "OK" to stdout on
...
def _build_env() -> dict:
    """Subprocess env:
      * prepend ``$CONDA_PREFIX/lib`` so subprocess native dependencies use
