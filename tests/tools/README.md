...
The repository-level CPU and packaging gate is available from the project
root:
...
It compiles the Python tree, runs every test marked `cpu`, builds a wheel, and
imports that wheel from an isolated environment outside the source checkout.
GitHub Actions runs the same command for pull requests and pushes to `main`.

`check_package.py` is the installed-distribution portion of that gate. It
guards the canonical `dmi` package structure and rejects legacy top-level
package paths that source-tree imports could otherwise hide.
