"""Prefill profiler benchmark comparing TransformerLens vs Hugging Face GPT-2."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Profile GPT-2 prefill across TransformerLens and Hugging Face baselines"
    )
    ...
    for idx, block in enumerate(blocks):
        ...
        if collect_hidden:
            ...
            def mlp_in_hook(
                ...
                if collector_enabled:
                    mlp_in_cache[_idx] = store_tensor(module_output)
            ...
            extra_hooks.append(block.mlp.register_forward_hook(mlp_in_hook))
            extra_hooks.append(block.mlp.register_forward_hook(mlp_out_hook))
...
def main() -> None:
    ...
    def tl_cache_prefill(prefill_tensor: torch.Tensor = prompt_tokens) -> Tuple[None, torch.Tensor]:
    ...
    def run_prefill(prefill_fn, prefill_tokens=prompt_tokens):
        run_prefill_loop(lambda: prefill_fn(prefill_tokens), args.steps)
    ...
    def warmup(prefill_fn, prefill_tokens=prompt_tokens):
        ...
        with torch.no_grad():
            for i in range(args.warmup):
                ...
                run_prefill_loop(lambda: prefill_fn(prefill_tokens), 1)
    ...
    warmup(tl_prefill)
    tl_elapsed = run_benchmark("transformer_lens", lambda: run_prefill(tl_prefill))
    timings["transformer_lens"] = {
        "duration": tl_elapsed,
        "tokens_per_second": total_prefill_tokens / tl_elapsed if tl_elapsed > 0 else float("inf"),
    }

    if device.type == "cuda":
        torch.cuda.empty_cache()

    warmup(tl_cache_prefill)
    tl_cache_elapsed = run_benchmark(
        "transformer_lens_cache", lambda: run_prefill(tl_cache_prefill)
    )
    timings["transformer_lens_cache"] = {
        "duration": tl_cache_elapsed,
        "tokens_per_second": total_prefill_tokens / tl_cache_elapsed if tl_cache_elapsed > 0 else float("inf"),
    }
    ...
    hf_hook_prefill, hf_hook_cleanup = setup_hf_decode_hook(
        hf_model,
    ...
    hf_hook_cpu_prefill, hf_hook_cpu_cleanup = setup_hf_decode_hook(
        hf_model,
        collect_hidden=args.collect_hidden,
        collect_attention=args.collect_attention,
        move_to_cpu=True,
    ...
    if monitoring_engine.async_enabled:
        ...
        if device.type == "cuda":
            torch.cuda.synchronize()
    main_async_prefill_elapsed, total_async_prefill_elapsed = run_async_benchmark(
        "hf_modified_hook_async",
        lambda: run_prefill(hf_modified_hook_async_prefill),
        monitoring_engine,
    )
    timings["hf_modified_hook_async"] = {
        "main_duration": main_async_prefill_elapsed,
        "total_duration": total_async_prefill_elapsed,
        "init_ms": engine_init_ms,
        "tokens_per_second_main": total_prefill_tokens / main_async_prefill_elapsed
        if main_async_prefill_elapsed > 0
        else float("inf"),
        "tokens_per_second_total": total_prefill_tokens / total_async_prefill_elapsed
        if total_async_prefill_elapsed > 0
        else float("inf"),
    }
    hf_hooked_model.monitoring_engine = None
