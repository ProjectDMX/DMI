"""Benchmark config request stride impact for HF Modified GPT-2."""
...
# EDIT ME: request stride values to test
REQUEST_STRIDES = [1, 2, 5,100]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Benchmark HF Modified GPT-2 config request stride impact"
    )
    ...
    parser.add_argument(
        "--output-dir",
        default="results/profile_hf_modified_request_stride",
...
def main() -> None:
    ...
    print(f"Request strides: {REQUEST_STRIDES}")
    ...
    for stride in REQUEST_STRIDES:
        cfg = MonitoringConfig(
            ...
            schedule=CaptureSchedule(
                ...
                capture_decode=True,
                request_stride=int(stride),
                request_offset=0,
                warmup_requests=0,
            ),
        )
        label = f"request_stride_{stride}"
    ...
    results_data = {
        "config": {
            ...
            "request_strides": REQUEST_STRIDES,
        },
        "results": results,
    }
