"""Simplified benchmark for HF Modified model (sync version, no hook/monitoring)."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Benchmark HF Modified GPT-2 (sync, no hook cache)"
    )
    ...
    parser.add_argument(
        "--profile-dir",
        default="results/profile_hf_modified_sync",
...
def main() -> None:
    ...
    hf_hooked_model.monitoring_engine = None  # Ensure no monitoring
    ...
    # === hf_modified functions (from line 689-733) ===
    ...
    timings = {
        "hf_modified": {
            "duration": elapsed,
            "tokens_per_second": total_decoded_tokens / elapsed if elapsed > 0 else float("inf"),
        }
    }
    ...
    print("\nTiming results:")
    print(
        f"- hf_modified: duration={elapsed:.4f}s "
        f"tokens/s={timings['hf_modified']['tokens_per_second']:.2f}"
    )
