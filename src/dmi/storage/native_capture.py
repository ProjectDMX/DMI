...
class NativeCaptureStorageConfig:
    ...
    # clock_skew_s; 0 fails at once. That outlasts a crashed predecessor
    # exactly when its TTL is at most lease_ttl_s + publish_timeout_s:
    # clock_skew_s cancels, since the wait adds it and a lagging replica sees
    # the row live that much longer. Set this explicitly for the first
    # restart after a predecessor above that threshold -- 20 s on these
    # defaults, so one on the native 30 s default (which processes predating
    # these knobs used) outlasts the default wait by 10 s.
