...
paths; always install with --no-build-isolation:
    ...
    pip install torch                        # install CUDA torch first
    pip install -e . --no-build-isolation    # compiles the native backend
...
Set SKIP_NATIVE_BUILD=1 to bypass steps 1-4 (CPU-only runners without nvcc).
...
# Stamp file: cmake writes this when clickhouse-cpp is ready.
...
class NativeBuildExt(_BuildExt):
    ...
    all real work happens in run().  Set SKIP_NATIVE_BUILD=1 to skip the
    native build (used by CPU-only CI runners without nvcc).
    """

    def run(self) -> None:
        if os.environ.get("SKIP_NATIVE_BUILD"):
            print("[DMI build] SKIP_NATIVE_BUILD set — skipping native backend.",
                  flush=True)
            return
    ...
    def build_extension(self, ext) -> None:  # noqa: ARG002
        pass  # placeholder; real artifacts come from the Makefile

    def copy_extensions_to_source(self) -> None:
        pass  # .so is already at the project root for editable installs

    def get_outputs(self) -> list[str]:
        return []  # don't advertise placeholder to pip
...
setup(
    ...
    # Placeholder extension triggers build_ext; sources=[] is intentional and
    # safe because build_extension() is a no-op.
    ext_modules=[
