...
RESULTS_DIR="${RESULTS_DIR:-exp/offline_e2e/results}"
LOCAL_ONLY_FLAG=()
...
run_baseline "hf_upper_bound (compile)" \
  "${PYTHON_BIN}" exp/offline_e2e/scripts/run_hf_upper_bound.py "${COMMON[@]}"
run_baseline "hf_upper_bound (eager)" \
  "${PYTHON_BIN}" exp/offline_e2e/scripts/run_hf_upper_bound.py "${COMMON[@]}" --disable-compile
run_baseline "hf_monitor (generate, eager)" \
  "${PYTHON_BIN}" exp/offline_e2e/scripts/run_hf_monitor.py "${COMMON[@]}"
run_baseline "hf_monitor_manual (compile)" \
  "${PYTHON_BIN}" exp/offline_e2e/scripts/run_hf_monitor_manual.py "${COMMON[@]}"
run_baseline "proj_dmi (compile) ${RING_MB}MB" \
  "${PYTHON_BIN}" exp/offline_e2e/scripts/run_proj_dmi.py "${COMMON[@]}" \
...
run_baseline "proj_dmi (eager) ${RING_MB}MB" \
  "${PYTHON_BIN}" exp/offline_e2e/scripts/run_proj_dmi.py "${COMMON[@]}" --disable-compile \
    --ring-payload-mb "${RING_MB}" --ring-pinned-mb "${RING_MB}" \
    --ring-task-entries 131072 \
    --drain-flush-payload-ratio "${FLUSH}" --drain-flush-task-ratio "${FLUSH}"
run_baseline "nnsight (eager)" \
  "${PYTHON_BIN}" exp/offline_e2e/scripts/run_nnsight.py "${COMMON[@]}"
