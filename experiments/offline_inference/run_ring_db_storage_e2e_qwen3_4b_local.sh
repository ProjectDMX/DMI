...
RESULTS_ROOT="${RESULTS_ROOT:-experiments/offline_inference/results/ring_db_storage_e2e}"
HOOK_SELECTION="${HOOK_SELECTION:-hidden-states,final_ln,logits}"
LOCAL_FILES_ONLY="${LOCAL_FILES_ONLY:---local-files-only}"

export CUDA_VISIBLE_DEVICES="${GPU}"

"${PYTHON_BIN}" experiments/offline_inference/scripts/run_ring_db_storage_e2e.py \
  --model "${MODEL}" \
