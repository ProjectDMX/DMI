...
BASELINE_CHOICES = ("hf_ideal", "hf_api", "torch_hooks", "proj_dmi", "proj_dmi_legacy")
...
def _compile_prefill_step(model: Any, *, output_hidden_states: bool, logits_to_keep: int | None = 1):
...
def _compile_decode_step(model: Any, *, output_hidden_states: bool, logits_to_keep: int | None = 1):
    wants_position_ids = _forward_accepts_position_ids(model)
...
def _measure_hf_ideal_decode(
    ...
    *,
    ...
    _wants_position_ids = _forward_accepts_position_ids(model)
    device_sync(device)
    ...
    for step_idx in range(decode_iters):
        ...
        if step_idx == decode_iters - 1:
            last_compute_ms = step_ms
            last_total_ms = step_ms
    total_ms = (time.perf_counter() - t_total0) * 1000.0
    print(f"  [hf_ideal] per-step ms: {['%.3f' % t for t in step_times_ms]}  sum={sum(step_times_ms):.3f}  total={total_ms:.3f}", flush=True)
...
def _measure_hf_api_prefill(
    ...
    *,
    ...
    t0 = time.perf_counter()
    if compiled_prefill is None:
        fwd_kwargs = _hf_forward_kwargs(model=model, **encoded, use_cache=True, output_hidden_states=True)
...
def _measure_hf_api_decode(
    ...
    *,
    ...
    step_total_times_ms: List[float] = []
    _wants_position_ids = _forward_accepts_position_ids(model)
    device_sync(device)
    t_total0 = time.perf_counter()
    decode_iters = max(int(decode_steps) - 1, 0)
    ...
    for step_idx in range(decode_iters):
        ...
        step_total_times_ms.append((step_end_t - step_t0) * 1000.0)
        if step_idx == decode_iters - 1:
            last_compute_ms = step_compute_times_ms[-1]
            last_total_ms = step_total_times_ms[-1]
    decode_total_compute_ms = (step_compute_t - t_total0) * 1000.0
    decode_total_total_ms = (step_end_t - t_total0) * 1000.0
    print(f"  [hf_api] per-step compute ms: {['%.3f' % t for t in step_compute_times_ms]}  sum={sum(step_compute_times_ms):.3f}", flush=True)
    print(f"  [hf_api] per-step total ms:   {['%.3f' % t for t in step_total_times_ms]}  sum={sum(step_total_times_ms):.3f}  seq_total={decode_total_total_ms:.3f}", flush=True)
    return (
        last_compute_ms,
        last_total_ms,
        last_total_ms,
...
def _measure_torch_hooks_decode(
    ...
    *,
    ...
    total_ms = (time.perf_counter() - t_total0) * 1000.0
    print(f"  [torch_hooks] per-step ms: {['%.3f' % t for t in step_times_ms]}  sum={sum(step_times_ms):.3f}  total={total_ms:.3f}", flush=True)
...
def _measure_dmi_decode(
    ...
    *,
    ...
    step_ts = timing_out.get("step_timestamps", [])
    if len(step_ts) >= 2:
        # step_ts[0]=prefill prepare, step_ts[1]=decode_0 prepare, ...
        # delta[0] = prefill time, delta[1..] = decode steps 0..N-2, last_step = decode N-1
        all_deltas = [(step_ts[i + 1] - step_ts[i]) * 1000.0 for i in range(len(step_ts) - 1)]
        prefill_delta = all_deltas[0]
        decode_deltas = all_deltas[1:] + [last_compute_ms]  # append last decode step
        print(f"  [proj_dmi] prefill={prefill_delta:.3f}ms  decode per-step ms: {['%.3f' % d for d in decode_deltas]}  sum={sum(decode_deltas):.3f}  seq_total={decode_total_compute_ms:.3f}  flush={flush_ms:.3f}", flush=True)
    return (
        last_compute_ms,
        last_total_ms,
...
def _measure_dmi_legacy_decode(
    ...
    *,
    ...
    print(f"  [proj_dmi] per-step compute ms: {['%.3f' % t for t in step_compute_times_ms]}  sum={sum(step_compute_times_ms):.3f}  seq_compute={decode_total_compute_ms:.3f}  total+flush={(resolve_end_t - t_total0) * 1000.0:.3f}", flush=True)
    if owns_engine:
        _close_proj_dmi_microbench(model, engine)
...
def add_step_breakdown_args(
    parser: argparse.ArgumentParser,
    *,
    include_baseline_arg: bool = True,
    ...
    elif default_baseline is None:
        raise ValueError("default_baseline is required when include_baseline_arg=False")
    else:
        parser.set_defaults(baseline=default_baseline)
...
def build_step_breakdown_parser(
    *,
    include_baseline_arg: bool = True,
    default_baseline: str | None = None,
) -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Prefill/decode-step breakdown microbenchmark.")
    add_step_breakdown_args(
        parser,
        include_baseline_arg=include_baseline_arg,
        default_baseline=default_baseline,
    )
    return parser


def run_step_breakdown(args: argparse.Namespace) -> None:
    if str(args.baseline) not in BASELINE_CHOICES:
    ...
    if args.baseline == "hf_ideal":
        ...
        if compile_enabled:
            compiled_decode = _compile_decode_step(model, output_hidden_states=False, logits_to_keep=1)
    elif args.baseline == "hf_api":
        model = _load_base_model(model_id, local_files_only=bool(args.local_files_only)).to(device).eval()
        if compile_enabled:
            compiled_decode = _compile_decode_step(model, output_hidden_states=True, logits_to_keep=1)
...
def main(
    ...
    *,
    include_baseline_arg: bool = True,
    default_baseline: str | None = None,
    default_baseline_label: str = "",
) -> None:
    parser = build_step_breakdown_parser(
        include_baseline_arg=include_baseline_arg,
        default_baseline=default_baseline,
    )
    args = parser.parse_args(argv)
    if default_baseline_label and not str(args.baseline_label).strip():
        args.baseline_label = default_baseline_label
    run_step_breakdown(args)
