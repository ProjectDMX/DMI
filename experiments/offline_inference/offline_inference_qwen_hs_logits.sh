...
RESULTS_DIR="${RESULTS_DIR:-experiments/offline_inference/results/offline_inference_qwen_hs_logits_$(date '+%Y%m%d_%H%M%S')}"
for model in ${MODELS_CSV//,/ }; do
  for dataset in ${DATASETS//,/ }; do
    ...
    bash experiments/offline_inference/scripts/run_all.sh \
      --model "${model}" \
      --datasets "${dataset}" \
      --baselines "hf_upper_bound_eager,hf_monitor_generate,hf_monitor_manual,proj_dmi,torch_hooks,nnsight" \
