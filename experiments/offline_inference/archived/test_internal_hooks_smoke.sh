...
RESULTS_DIR="${PROJECT}/experiments/offline_inference/results/test_internal_hooks_${RUN_TAG}"
mkdir -p "${RESULTS_DIR}"
...
for sample_file in "${SHAREGPT_SAMPLE}" "${WILDCHAT_SAMPLE}"; do
  ...
  run_cmd "internal_hooks/torch_hooks_eager/${dataset}" \
    python scripts/run_torch_hooks.py "${COMMON[@]}" --hook-selection "${INTERNAL_HOOKS}" --disable-compile

  run_cmd "internal_hooks/nnsight_eager/${dataset}" \
    python scripts/run_nnsight.py "${COMMON[@]}" --hook-selection "${INTERNAL_HOOKS}" --disable-compile

  run_cmd "internal_hooks/proj_dmi_compile/${dataset}" \
    python scripts/run_proj_dmi.py "${COMMON[@]}" --proj-dmi-mode ring_null \
