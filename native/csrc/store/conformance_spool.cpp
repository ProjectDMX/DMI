...
int main() {
  ...
  while (std::getline(std::cin, line)) {
    ...
    config.root = root;
    config.max_bytes = static_cast<uint64_t>(jc::FindInt(line, "max_bytes"));
    ...
    if (op == "stage") {
      ...
      const dmi_store::SpoolStatus st = spool.Stage(
          ...
          static_cast<uint64_t>(jc::FindInt(line, "created_at_ns")),
          static_cast<uint64_t>(jc::FindInt(line, "record_count")),
