"""Hugging Face adapter and monitored generation entry points.

This module owns Hugging Face-specific orchestration while framework-neutral
engine, hook, and transport code remains in their respective DMI packages.
...
from ...transport import ring as ring_transport
from ..base import BackendAdaptor
...
# legacy ``_make_model_shape(model)`` wrapper was deleted after its only caller
...
# Preferred descriptive name for new integrations.  ``HFAdaptor`` remains the
# defining class for compatibility with existing callers and serialized names.
HuggingFaceAdapter = HFAdaptor
